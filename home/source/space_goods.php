...
if($id) {
	...
	$query8 = $_SGLOBAL['db']->query("SELECT bf.message, bf.target_ids, bf.magiccolor, b.* FROM ".tname('goods')." b 
				LEFT JOIN ".tname('goodsfield')." bf ON bf.goodsid=b.goodsid where b.goodsid='6'
				ORDER BY b.dateline DESC ");
	$wei = $_SGLOBAL['db']->fetch_array($query8);
	//ÊµÃû
	realname_get();

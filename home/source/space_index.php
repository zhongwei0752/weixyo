...
$goodslist = array();
	...
	$query = $_SGLOBAL['db']->query("SELECT bf.message, bf.target_ids, bf.magiccolor, b.* FROM ".tname('goods')." b 
				...
				ORDER BY b.viewnum DESC LIMIT 0,10");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$value['subject'] = getstr($value['subject'], 13, 0, 0, 0, 0, -1);
...
//×î½ü·Ã¿ÍÁÐ±í
$visitorlist = array();
$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('visitor')." WHERE uid='$space[uid]' ORDER BY dateline DESC LIMIT 0,8");
while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	if($value['vusername']) {

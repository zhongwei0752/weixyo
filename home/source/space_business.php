...
if(!defined('IN_UCHOME')) {
	...
	}elseif($space['profilestatus']=='0'&&$space['namestatus']=='1'&&$space['cheak']=='0'){
...
$theurl = "space.php?do=business&op=$_GET[op]";
...
if($_GET['op'] == 'base') {
	...
	if(empty($space['companyintroduce'])){
		...
		//²úÉúfeed
		if(ckprivacy('business', 1)) {
			feed_add('business', cplang('feed_profile_update_base'));
		}
	...
	if(empty($_SCONFIG['namechange'])) {
		$_GET['namechange'] = 0;//²»ÔÊÐíÐÞ¸Ä
			...
			if(submitcheck('nextsubmit')) {
				...
				showmessage('update_on_successful_individuals', 'space.php?do=business', 0);
				}else{
...
include template("space_business");

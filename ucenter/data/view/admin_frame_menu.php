...
<div class="menu">
	<ul id="leftmenu">
		...
		<? if($user['allowadminsetting'] || $user['isfounder']) { ?><li><a href="admin.php?m=setting&a=mail" target="main">邮件设置</a></li><? } ?>
		...
		<? if($user['isfounder']) { ?><li><a href="admin.php?m=feed&a=ls" target="main">数据列表</a></li><? } ?>
		<? if($user['allowadmincache'] || $user['isfounder']) { ?><li><a href="admin.php?m=cache&a=update" target="main">更新缓存</a></li><? } ?>
		<? if($user['isfounder']) { ?><li><a href="admin.php?m=plugin&a=filecheck" target="main">插件</a></li><? } ?>
	</ul>
</div>
